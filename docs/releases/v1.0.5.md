...
- Helper and agent configs now support explicit DNS timeout, DNS cache TTL, and
  DNS in-flight limits so unresponsive resolvers fail predictably instead of
...
- WARP-routed exits still depend on the local WARP/WireProxy route for reaching
  the host; direct exits can continue working if that WARP route fails.
