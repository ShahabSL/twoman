...
TWOMAN_TLS_HANDSHAKE_TIMEOUT_SECONDS="${TWOMAN_TLS_HANDSHAKE_TIMEOUT_SECONDS:-}"
...
TWOMAN_DNS_QUERY_TIMEOUT_SECONDS="${TWOMAN_DNS_QUERY_TIMEOUT_SECONDS:-}"
TWOMAN_DNS_CACHE_TTL_SECONDS="${TWOMAN_DNS_CACHE_TTL_SECONDS:-}"
TWOMAN_DNS_MAX_INFLIGHT="${TWOMAN_DNS_MAX_INFLIGHT:-}"
...
if [ ! -f "${CONFIG_PATH}" ]; then
  ...
  export TWOMAN_TLS_HANDSHAKE_TIMEOUT_SECONDS
  export TWOMAN_IDLE_REPOLL_CTL TWOMAN_IDLE_REPOLL_DATA TWOMAN_HTTP2_CTL TWOMAN_HTTP2_DATA
  export TWOMAN_DNS_QUERY_TIMEOUT_SECONDS TWOMAN_DNS_CACHE_TTL_SECONDS TWOMAN_DNS_MAX_INFLIGHT
...
if (tls_timeout := maybe_float("TWOMAN_TLS_HANDSHAKE_TIMEOUT_SECONDS")):
    config["tls_handshake_timeout_seconds"] = tls_timeout
if (dns_timeout := maybe_float("TWOMAN_DNS_QUERY_TIMEOUT_SECONDS")):
    config["dns_query_timeout_seconds"] = dns_timeout
if (dns_ttl := maybe_float("TWOMAN_DNS_CACHE_TTL_SECONDS")):
    config["dns_cache_ttl_seconds"] = dns_ttl
if (dns_max_inflight := maybe_int("TWOMAN_DNS_MAX_INFLIGHT")):
    config["dns_max_inflight"] = dns_max_inflight
data_profile = {}
