...
TWOMAN_DNS_QUERY_TIMEOUT_SECONDS="${TWOMAN_DNS_QUERY_TIMEOUT_SECONDS:-10}"
TWOMAN_DNS_CACHE_TTL_SECONDS="${TWOMAN_DNS_CACHE_TTL_SECONDS:-60}"
TWOMAN_DNS_MAX_INFLIGHT="${TWOMAN_DNS_MAX_INFLIGHT:-8}"
...
"${SSH_CMD[@]}" "${TWOMAN_SERVER_USER}@${TWOMAN_SERVER_HOST}" "mkdir -p '${TWOMAN_SERVER_DIR}/systemd' '${TWOMAN_SERVER_DIR}/logs'"
...
CONFIG_JSON="$(cat <<EOF
{
  ...
  "dns_query_timeout_seconds": ${TWOMAN_DNS_QUERY_TIMEOUT_SECONDS},
  "dns_cache_ttl_seconds": ${TWOMAN_DNS_CACHE_TTL_SECONDS},
  "dns_max_inflight": ${TWOMAN_DNS_MAX_INFLIGHT},
  ...
  "log_path": "${TWOMAN_SERVER_DIR}/logs/agent.log",
  "event_log_path": "${TWOMAN_SERVER_DIR}/logs/agent-events.ndjson"
}
EOF
)"
