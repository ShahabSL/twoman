...
ARCHIVE_PATH="${TWOMAN_CLIENT_ARCHIVE_PATH:-$(dirname "${OUT_DIR}")/twoman-cli-linux-amd64.tar.gz}"

mkdir -p "${OUT_DIR}"
(
  cd "${ROOT}/helper-agent"
  CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags "-s -w" -o "${OUT_DIR}/twoman-helper-agent" .
)
(
  cd "${ROOT}/client-cli"
  CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags "-s -w" -o "${OUT_DIR}/twoman" .
)
