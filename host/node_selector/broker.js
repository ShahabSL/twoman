...
const CIPHER_SUITE_HMAC_SHA256_CTR = "hmac-sha256-ctr-v1";
const CIPHER_SUITE_AES_256_CTR = "aes-256-ctr-v2";
const SUPPORTED_CIPHER_SUITES = new Set([CIPHER_SUITE_HMAC_SHA256_CTR, CIPHER_SUITE_AES_256_CTR]);
...
class BrokerState {
  constructor(config) {
    ...
    this.agentDataUploadProfile = {
      ...
      flushDelaySeconds: Math.max(0, Number(config.agent_data_upload_flush_delay_seconds ?? 0.006))
    };
  ...
  ensurePeer(role, peerLabel, peerSessionId, targetAgentPeerLabel = "") {
    ...
    if (!peer) {
      for (const existingPeer of Array.from(this.peers.values())) {
        if (
          existingPeer.role === role &&
          existingPeer.peerLabel === peerLabel &&
          existingPeer.peerSessionId !== peerSessionId
        ) {
...
const FALLBACK_CAMOUFLAGE_404_HTML = [
  "<!doctype html>",
  '<html lang="en">',
  "<head>",
  '<meta charset="utf-8" />',
...
let camouflageNotFoundHtml = null;
...
function loadCamouflageNotFoundHtml() {
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Fall through to the generated fallback. Public error masking must not
      // make the broker unavailable if the static site has not been deployed.
    }
  }
...
// Keep these intervals referenced. CloudLinux Node Selector runs on Passenger,
// and unref'd background timers allow the broker event loop to go idle after a
// request completes, which makes Passenger tear the process down mid-session.
setInterval(() => {
  state.cleanup();
}, 10000);
